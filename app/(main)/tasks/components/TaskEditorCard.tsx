...
export default function TaskEditorCard({
  ...
  return (
    <div id={id} className="flex items-start justify-center space-x-2">
      <Collapsible
        open={isOpen && !isCollapsed}
        onOpenChange={setIsOpen}
        className="w-full max-w-[750px] space-y-2 break-inside-avoid"
      >
        <Card
          key={id}
          className="max-w-[750px] w-full break-inside-avoid focus-within:shadow-lg focus-within:shadow-primary/50 transition-shadow duration-500"
        >
          <TooltipProvider>
                    ...
                    <TooltipTrigger>
                      <Button
                        ...
                        onClick={() => {
                          ...
                          unassignSubtask(parentIndex, index);
                        }}
                      >
            ...
            {children.map((subtask, subtaskIndex) => {
              return (
                <SortableTaskCard
                  ...
                  children={[]}
                  rootTasks={rootTasks}
                  unassignSubtask={unassignSubtask}
                  parentIndex={index}
                  isSubtask
                />
              );
