...
def test_null_task_name(test_db, test_tasks):
    new_task = Task(name=None, project_id=test_tasks)
...
def test_duplicate_project_names(test_db, test_tasks):
    ...
    new_task = Task(name="Task 1", project_id=test_tasks)
    new_task2 = Task(name="Task 1", project_id=test_tasks)
    test_db.session.add(new_task)
    test_db.session.commit()
...
def test_multiple_project_tasks(test_db, test_tasks):
    new_task = Task(name="Task 1", project_id=test_tasks)
    new_task2 = Task(name="Task 2", project_id=test_tasks)
    new_task3 = Task(name="Task 3", project_id=test_tasks)
    test_db.session.add(new_task)
    test_db.session.add(new_task2)
    test_db.session.add(new_task3)
    test_db.session.commit()

    assert test_db.session.query(Task).count() == 3
