...
def get_project_via_id(project_id: str, db_session: DBSessionDep):
    return get_project(db_session, project_id)


@router.get("/enrolments/{user_id}", response_model=List[ProjectInfo])
def get_enrolled_projects_of_a_user(user_id: str, db_session: DBSessionDep):
...
def enrol_multiple_users_to_a_project(
    ...
    enrol_users_to_project(db_session, user_ids.user_ids, project_id)
    return {"message": "Success!! Users enrolled to the project."}


@router.put("/{project_id}")
def update_project_via_id(project_id: str, update_data: dict, db_session: DBSessionDep):
    update_project(db_session, project_id, update_data)
    return {"message": "Success!! Project updated."}


@router.delete("/{project_id}")
def delete_project_via_id(project_id: str, db_session: DBSessionDep):
...
# def get_all_projects_via_assignment(assignment_id: str, db_session: DBSessionDep):
#     return get_projects_by_assignment(db_session, assignment_id)
